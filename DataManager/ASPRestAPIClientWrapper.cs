...
namespace iRLeagueManager.Data
{
    public class ASPRestAPIClientWrapper : NotifyPropertyChangedBase, ILeagueDbServiceClient, IDisposable
    {
        ...
        {
            var requestMsg = new DELItemsRequestMessage()
            {
                databaseName = request.databaseName,
                password = request.password,
                requestItemIds = request.requestItemIds,
                requestItemType = request.requestItemType,
                requestResponse = request.requestResponse,
                userName = request.userName
            };
            var result = await client.PostAsXmlAsync(requestUri, requestMsg);

            if (result.IsSuccessStatusCode)
            {
                var contentMsg = await result.Content.ReadAsAsync<DELItemsResponseMessage>();

                var content = new DELItemsResponseMessage()
                {
                    databaseName = contentMsg.databaseName,
                    status = contentMsg.status,
                    success = contentMsg.success,
                    Username = contentMsg.Username
                };
                if (content != null)
                    return content;
            }

            throw new WebException("Could not delete entries.");
        }
